...
void connect(BTreeNode* left, BTreeNode* right, BTreeNode* parent){
	//split되어 새로 생긴 internal 노드(this)에서 호출함. 
	//1. this의 keys, child 채우고 형제 노드 keys, child 빼줌. 
		//이때, right->child[0] key값을 keys[0]에 넣을지 말지는 left, right의 타입 보고 결정.
		//leaf와 연결하는 경우에는 넣어주고, 
		//internal과 연결하는 경우에는 넣지 않음. 
	//2. 위와 연결 
	//3. right를 child 배열에 넣어줌. (새로 생겼으니까)
	//4. 위로 올려보낼 key
	//왼쪽/오른쪽 자식과 이들의 원래 부모 노드 (새로 생긴 애의 형제 노드가 될 노드) 정보 가지고 들어감. 
